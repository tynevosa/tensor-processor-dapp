...
const OutputBar: React.FC<OutputProps> = ({ question, queChange, output, showResult }) => {

  const nodes = question === null ? [] : Object.values(question)

  return (
    <div className='absolute z-2  mt-24 right-side '>
      <div className=" mt-8 max-w-[330px] w-full">
        <div className='border border-[#1D1F29] bg-[#101016]  bg-opacity-95'>
          <div className='flex text-white border border-[#1D1F29] px-2 pt-3 pb-3 font-bold'><FaBolt className='text-[24px] text-[yellow] pr-2' />   Global Input</div>
          <div className='p-4'>
        ...
        <div className='border border-[#1D1F29] mt-8 bg-[#101016] bg-opacity-95'>
          <div className='flex justify-between text-white border border-[#1D1F29] px-2 pt-3 pb-3 font-bold gap-2'>
            <div className='flex items-center'><FaBolt className='text-[24px] text-[yellow]' /> Result</div>
            <FaExpandArrowsAlt className="text-[white] cursor-pointer" onClick={showResult} /> 
          </div>
          <div className='p-6 text-white text-[14px] max-h-[250px] overflow-y-auto'>
            {output}
          </div>
        </div>
      </div>
